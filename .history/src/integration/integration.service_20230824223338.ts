...
import { Block, Provider, Wallet,ethers, keccak256, solidityPacked , encodeBytes32String , hexlify, } from 'ethers';
...
export class IntegrationService {
  ...
  private contractAddress = "0x0165878A594ca255338adfa4d48449f69242Eb8F"
  private abi = [
    {
      "inputs": [],
  ...
  async makeSignature(signers: any, messageHash: string) {
    const signatures: any[] = await Promise.all(signers.map(async (wallet: any) => {
      const bytesMessageHash = hexlify(messageHash)
  ...
  console.log(adminsigner, "fsuresfhjsdk uerysk jreiuer reiureui ");
  
  const adminAddress = await adminsigner.getAddress();
  console.log(adminAddress, "addminaddress");
  ...
  const hash = await this.makeHash(adminAddress, timestamp);
  console.log(hash, "hashkdkskjdskjdskjskjdskjdskjdskjkjfkjfkjfkjfkjf");
  
  const signatures = await this.makeSignature([adminsigner], hash)
  console.log(signatures, "signatures");
  
  const data = await this.contract.addDiagnosticStudy(orderNumber, "cancer", timestamp, 
    signatures
    )
  console.log(data , "data kjerkjrekjrekjkjrekjkjrekjrej");
  // return 0; 
  // return data
  // }catch(error){ 
  //   console.log(error);
