...
export class IntegrationController {
  ...
  async getOtherTreatment(@Param('startDate') startDate: number): Promise<{ treatmentCategory: string, endDate: number; timestamp: number }> {
    return this.physicalFindingService.getOtherTreatment(startDate);
  }
    ...
    @Get(':orderNumber')
    async getDianosticStudy(@Param("orderNumber") orderNumber: number) {
        return this.integrationService.getDianosticStudy(orderNumber)
    }
}
