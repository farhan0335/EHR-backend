...
async getDiagnosticStudies(patientaddress : string){
  ...
  async getPhysicalFinding(patientAddress : string){
    try {
      const physicalFinding = await this.contract.getPhysicalFinding(patientAddress);
  ...
  async getOperationRecord(patientAddress : string) {
    try {
      const operationRecord = await this.contract.getOperationRecord(patientAddress);
...
//      set Other Treatment
  async addOtherTreatment(adminprivateKey: string, treatmentCategory: string, startDate: number, endDate: number, : number): Promise<void> {
    // try {
      const adminsigner = new ethers.Wallet(adminprivateKey, this.provider)
      const adminAddress = await adminsigner.getAddress()
      const doctor_role = await this.contract.DOCTOR_ROLE();
