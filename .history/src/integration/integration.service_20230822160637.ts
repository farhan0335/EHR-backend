...
export class IntegrationService {
  ...
  async getdiagnosis(patientaddress: string) {
   ...
   async addDiagnosis(_diagnosesCode : string, _date: number, adminsigner : any) : Promise<void> {
    const addminAddress = await adminsigner.getaddress();
    const timestamp = Date.now()
    const doctor_role = await this.contract.DOCTOR_ROLE();
    await this.contract.grantRole(doctor_role, adminsigner)
