...
export class IntegrationController {
    ...
    @Post('add-diagnoses')
    async addDiagnoses(@Body() requestBody: { adminprivateKey: string, diagnosescode: string }) { @Post('set-physical-finding')
  // async setPhysicalFinding(@Body() requestBody: any) {
  //   try {
  //     const { adminPrivateKey, date, value1, value2 } = requestBody;
  //     await this.ehrService.setPhysicalFinding(adminPrivateKey, date, value1, value2);
  //     return { message: 'Physical finding set successfully' };
  //   } catch (error) {
  //     console.error(error);
  //     return { error: 'Error setting physical finding' };
  //   }
  // }
