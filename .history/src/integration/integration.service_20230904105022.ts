...
export class IntegrationService {
  ...
  async makeSignature(signers: any, messageHash: string) {
    const signatures: any[] = await Promise.all(signers.map(async (wallet: any) => {
      ...
      // const bytesMessageHash = '0x' + messageHash.substring(2).padStart(64, '0');
      // console.log(messageHash, "messageHash");
      // console.log(bytesMessageHash, "bytesMessageHash");
  ...
  async addDiagnosesStudy(privatekey: string, orderNumber: number, subCategory: string) {
    ...
    ZZconst hash = await this.makeHash(adminAddress, timestamp);
    ...
    // console.log(data_, "data_data_data_");
  ...
  async getDianosticStudy(orderNumber: number): Promise<any> {
    // try {
      const diagnosticStudy = await this.contract.getDiagnosticStudies(orderNumber);
      return diagnosticStudy;
    // } catch (error) {
    //   throw new error("error fetching dianostic study")
    // }
  }
  ...
  //    Get Laboratory Test
  async getLaboratoryTest(date: number): Promise<{ testOrder: string, testCode: string, testName: string }> {
    try {
      const laboratoryTest = await this.contract.getLaboratoryTest(date);
      return {
        testOrder: laboratoryTest.testOrder,
        testCode: laboratoryTest.testCode,
        testName: laboratoryTest.testName,
      };
    } catch (error) {
      console.log(error);
  ...
  async getMedicationInjection(date: number): Promise<{ injectionType: number, medication: string, administered: boolean; timestamp: number }> {
    try {
      const medicationInjection = await this.contract.getMedicationInjection(date);
      return {
        injectionType: medicationInjection.injectionType,
        medication: medicationInjection.medication,
        administered: medicationInjection.administered,
        timestamp: medicationInjection.timestamp.toNumber(),
      };
    } catch (error) {
      console.log(error);
      throw new Error("Error getting medication injection");
    }
  }
//      set Other Treatment
...
//      Get Other Treatment
  async getOtherTreatment(startDate: number): Promise<{ treatmentCategory: string, endDate: number; timestamp: number }> {
    try {
      const otherTreatment = await this.contract.getOtherTreatment(startDate);
      return {
        treatmentCategory: otherTreatment.treatmentCategory,
        endDate: otherTreatment.endDate.toNumber(),
        timestamp: otherTreatment.timestamp.toNumber(),
      };
    } catch (error) {
      console.log(error);
