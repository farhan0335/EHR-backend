...
import { Block, Provider, Wallet,ethers, keccak256, solidityPacked , encodeBytes32String , hexlify,toUtf8Bytes } from 'ethers';
...
export class IntegrationService {
  ...
  private contract = new ethers.Contract(this.contractAddress, JSON.stringify(this.abi));
  ...
  async makeHash(recipient: any, timestamp: any) {
    ...
    const messageHashe = ethers.keccak256(toUtf8Bytes(concatenatedArgs));
    return messageHashe;
  }
  ...
  const data = await this.contract.connect(adminsigner).addDiagnosticStudy(orderNumber, subCategory, timestamp, 
    signatures
    )
