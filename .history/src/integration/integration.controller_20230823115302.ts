...
export class IntegrationController {
    ...
    @Post('add-diagnoses')
    async addDiagnoses(
        @Body() requestBody: { adminprivateKey: string; diagnosescode: string },
    ) {
        const { adminprivateKey, diagnosescode } = requestBody;
        const wallet = new ethers.Wallet(
            adminprivateKey,
            this.integrationService.getProvider(),
        );
    ...
    async setPhysicalFinding(@Body() requestBody: any) {
        try {
            ...
            await this.integrationService.setphysicalFinding(adminprivateKey,
                date,
                value1,
                value2,
            );
    ...
    async addOperationRecord(@Body() requestBody: any) {
        try {
            ...
            await this.integrationService.addOperationRecord(
                adminprivateKey,
                date,
                field1,
                field2,
    ...
    async addLaboratoryTest(@Body() requestBoby: any) {
        try {
            const { adminprivateKey, date, testOrder, testCode, testName } =
                requestBoby;
            await this.integrationService.addLaboratoryTest(
                adminprivateKey,
                date,
                testOrder,
                testCode,
