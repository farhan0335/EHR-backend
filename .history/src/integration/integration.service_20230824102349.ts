...
import { Block, ethers, Provider, Wallet, keccak256, solidityPacked} from 'ethers';
...
export class IntegrationService {
  ...
  async makeSignature(signers: any, messageHash: string) {
    const signatures: any[] = await Promise.all(signers.map(async (wallet: any) => {
      const bytesMessageHash = new Uint8Array(Buffer.from(messageHash.slice(2), 'hex'));
  ...
  async setphysicalFinding(adminprivateKey: string, date: number, value1: number, value2: number): Promise<void> {
    try {
      ...
      // const hash = await makeHash(adminAddress, date);
      // const signatures = await this.makeSignature([adminsigner], hash)
  ...
  async addDiagnosesStudy(adminprivateKey: string, orderNumber: number, studyDescription: string) {
    // try
    // {
    ...
    const data = await this.contract.addDiagnosticStudy(adminprivateKey, studyId, studyDescription, timestamp, signatures)
    console.log(data);
    return 0;
    // return data
    // }catch(error){
