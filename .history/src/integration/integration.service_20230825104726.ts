...
import { Block, Provider, Wallet,ethers, keccak256, solidityPacked , encodeBytes32String , hexlify,toUtf8Bytes ,arrayify } from 'ethers';
...
export class IntegrationService {
  ...
  async makeSignature(signers: any, messageHash: string) {
    const signatures: any[] = await Promise.all(signers.map(async (wallet: any) => {
      ...
      let signature = wallet.signMessage(uti(bytesMessageHash));
