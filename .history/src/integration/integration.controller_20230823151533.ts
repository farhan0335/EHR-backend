...
export class IntegrationController {
  ...
  async addDiagnosesStudy(@Body() requestBoby : any){
        try
        {
            const {adminprivateKey, studyId, studyDescription } = requestBoby;
            await this.integrationService.addDiagnosesStudy(adminprivateKey, studyId, studyDescription)
            return {message : 'Diagnoses study added sucessf'}
        }catch(error){}
  }
}
