...
export class IntegrationService {
  ...
  async getdiagnosis(patientaddress: string) {
   ...
   async addDiagnosis(diagnosesCode : string, date: number, timestamp:number,  signature : string[]) : Promise<void> {
    ...
    if(!isadmin){
      throw new Error("Only admiinistrator can add diagnoses.");

    }
   } 
   async  makeSignature(signer : any, message : string){
    const signatures : any[] = await Promise.all(signer.map(async (Wallet: any)=> {
      let signature = Wallet.signMessage(ethers.utils.arrayif(messageHash));
      return signature
    }))
    return signatures; 
   }
   export function makeHash(recipient : any, timestamp : any) {
    let concatenatedArgs = ethers.utils.solidityPack(["address", "uint256"], [recipient, timestamp])
    const messageHash = ethers.u
