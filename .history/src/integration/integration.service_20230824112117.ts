...
import { Block, Provider, Wallet, keccak256, solidityPacked , encodeBytes32String} from 'ethers';
...
export class IntegrationService {
  ...
  makeHash(recipient: any, timestamp: any) {
    let concatenatedArgs = ethers.solidityPacked(["address", "uint256"], [
      recipient,
      timestamp
    ])
    const messageHashe = ethers.keccak256(concatenatedArgs);
    return messageHashe;
  }
  async makeSignature(signers: any, messageHash: string) {
    const signatures: any[] = await Promise.all(signers.map(async (wallet: any) => {
      // const bytesMessageHash = encodeBytes32String(messageHash)
      const bytesMessageHash = '0x' + messageHash.substring(2).padStart(64, '0');
      console.log(bytesMessageHash , "bytesMessageHash");
      
      let signature = wallet.signMessage(bytesMessageHash);
  ...
  async addDiagnosis(diagnosesCode: string, adminsigner: any): Promise<void> {
    try {
      ...
      const hash = this.makeHash(adminAddress, timestamp);
  ...
  async setphysicalFinding(adminprivateKey: string, date: number, value1: number, value2: number): Promise<void> {
    try {
      ...
      const hash = this.makeHash(adminAddress, date);
      const signatures = await this.makeSignature([adminsigner], hash)
  ...
  async addDiagnosesStudy(adminprivateKey: string, orderNumber: number, subCategory: string) {
  // try
  // {
  ...
  const timestamp = Date.now();
  const hash = this.makeHash(adminAddress, timestamp);
  const signatures = this.makeSignature([adminsigner], hash)
  console.log(signatures, "signa");
  
  // const data = await this.contract.addDiagnosticStudy(orderNumber, subCategory, timestamp, signatures)
  // console.log(data);
  return 0;
  // return data
  // }catch(error){
